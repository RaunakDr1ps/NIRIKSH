...
import { Cpu, Thermometer, Wind, Gauge, Activity, Box } from 'lucide-react';
...
const Engine3D = lazy(() => import('@/components/engine/Engine3D'));
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 glass-panel p-6 relative overflow-hidden">
        ...
        <div className="lg:col-span-2 glass-panel p-4 relative overflow-hidden">
          <div className="flex items-center gap-2 mb-3">
            <Box className="w-4 h-4 text-hud-blue" />
            <span className="text-[10px] text-gray-500 font-mono tracking-wider">3D TURBOJET</span>
          </div>
          <Suspense
            fallback={
              <div className="w-full flex items-center justify-center" style={{ minHeight: 220 }}>
                <span className="text-[10px] text-gray-600 font-mono tracking-wider">LOADING 3D VIEW...</span>
              </div>
            }
          >
            <Engine3D
              compressorHealth={health?.compressorHealth ?? 0.95}
              combustorHealth={health?.combustorHealth ?? 0.92}
              turbineHealth={health?.turbineHealth ?? 0.88}
              overallHealth={health?.overallHealth ?? 0.95}
            />
          </Suspense>
        </div>
      </motion.div>
